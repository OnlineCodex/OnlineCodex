...
public class IMRelicLeviathanDreadnought extends Eintrag {
    ...
    public void refreshen() {
        final int dcw = (o1.getAnzahl("Leviathan siege claw & meltagun"));

        if (dcw == 2) {
            o1.setPreis("Leviathan siege claw & meltagun", (getPts("Leviathan siege claw (pair)")) / 2.0 + getPts("Meltagun"));
        } else {
            o1.setPreis("Leviathan siege claw & meltagun", getPts("Leviathan siege claw (single)") + getPts("Meltagun"));
        }

        final int cf = (o1.getAnzahl("Leviathan siege drill & meltagun"));

        if (cf == 2) {
            o1.setPreis("Leviathan siege drill & meltagun", (getPts("Leviathan siege drill (pair)")) / 2.0 + getPts("Meltagun"));
        } else {
