...
public abstract class BuildaVater extends BuildaPanel implements ActionListener, ItemListener, BuildaSTK {
	...
	public Hashtable<String, Integer> informationList = new Hashtable<String, Integer>(); // anzahl der vorkommen des
																							// Strings
	...
	public String[] Befestigungen = new String[] { "", "[BK] Aegis-Verteidigungslinie",
			"[BK] Himmelsschild-Landeplattform", "[BK] Imperiale Bastion", "[BK] Festung der Erlösung",
			"[BK] Vergelter Waffenbatterie", "[BK] Feuersturm Abwehrstellung", "[BK] Aquilla-Artilleriebunker",
			"[BK] Deflektor-Relaisnetzwerk", "[BK] Deflektor-Schildgenerator", "[BK] Ehrwürdiges Imperium 1",
			"[BK] Befestigte Stellung", "[BK] Imperialer Bunker", "[BK] Imperiale Verteidigungslinie",
			"[BK] Imperiale Verteidigungsstellung", "[BK] Promethium-Relaisleitungen",
			"[BK] Imperiales Verteidigungsnetzwerk", "", "Plasma Obliterator" };
	public String[] LordofWar;
	public Superformation superformation;
	public Formation formation;
	public boolean fcountBool = false; // Im Falle einer Subformation mit nur einer Auswahl, kann die Auswahl mehrmals
										// gewählt werden und zählt mehrfach.
	...
	// Bei Subformationen von Superformationen werden die Checkboxes für spezielle
	// Armeevarianten versteckt. Muss in der Volk-Datei überschrieben werden
	public void hideVolkCheckboxes() {

	}

	// Fügt die Formationen ein, die nur in Superformationen sichtbar sind. Muss in
	// der Volk-Datei überschrieben werden
		...
		if (saveText.contains(SAVETEXT_SUBDETACHMENTTRENNER)) {
			...
			for (int i = 0; i < myChooserGruppen.size(); ++i) {
				...
				if (myChooserGruppen.elementAt(i).getKategorie() != 7
					...
					// Lädt erst die Einträge, die Spezialauswahlen aktivieren, danach die
					// Standard-Kategorie.
	...
	public void loadElement(Element e) {
		...
		final int childrenMax = myChooserGruppen.size() > childrenChooserGruppe.getLength()
				? childrenChooserGruppe.getLength()
				: myChooserGruppen.size();
	...
	private void legKeyAn(String s) {
		...
		} // muss immer aufgerufen werden, ansonsten nullPointerException falls der key
			// nicht vorhanden war...
	}
	...
	/**
	 * Legtz für den letzten Choser eine eigene Einheitenliste fest, ohne
	 * Leereintrag
	 *
	 * @param kategorie Schlachtfeldrolle
	...
	public void fillChooserSpace() {
		if (superformation == null) {
			...
			for (int i = 0; i < myChooserGruppen.size(); i++) {
				...
				if (myChooserGruppen.get(i).alleEinträge.length == 1
						|| (myChooserGruppen.get(i).minAnzahl == 0 && myChooserGruppen.get(i).maxAnzahl == 0)) {// Falls
																												// die
																												// Gruppe
																												// nur
																												// einen
																												...
																												// Bild
																												// geschoben
					myChooserGruppen.get(i).panel.setLocation(outOfPanel,
							myChooserGruppen.get(i).panel.getLocation().y);
				} else {
	...
	public int getPts(String key) {
		return Optional.ofNullable(pointValues.get(sanitizeKey(key))).orElseGet(() -> {
	...
	// Passt Formationen und Kontingente an, die in einem Supplement nutzbar sind.
	// Muss in der Volk-Datei überschrieben werden
