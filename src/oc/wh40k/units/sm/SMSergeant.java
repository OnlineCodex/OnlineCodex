...
public class SMSergeant extends RuestkammerVater {
	...
	public void refreshen() {
		...
		if(isLegion){
			am.setAktiv(LotDDetachment);
			
			if(LotDDetachment){
				if(am.isSelected()){
					if(amBool==false){
						amBool=true;
						BuildaHQ.addToInformationVectorGlobal("SMAnimusMalorum", 1);
					}
				} else{
					if(amBool==true){
						amBool=false;
			...
			if(BuildaHQ.getCountFromInformationVectorGlobal("SMAnimusMalorum")>1 && am.isSelected()) {
				uniqueError=true;
			}else{
				uniqueError=false;
			}
